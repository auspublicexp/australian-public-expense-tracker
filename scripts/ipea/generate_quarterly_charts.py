...
def financial_year_quarter_folder(source_name):
    """Return the public Australian financial-year quarter for an IPEA source name.

    IPEA source files use calendar quarters (for example 2026q02), while APET
    publishes Australian financial-year quarters (FY2025-26 Q4). Keeping the
    source filename unchanged preserves the audit trail; only generated output
    folders use the public reporting label.
    """
    match = re.match(r"^(\d{4})q0?([1-4])", source_name.lower())
    if not match:
        raise ValueError(
            f"Could not determine the financial-year quarter from {source_name!r}"
        )
    ...
    fy_start, fy_end, fy_quarter = financial_mapping[calendar_quarter]
    return f"FY{fy_start}-{str(fy_end)[-2:]}_Q{fy_quarter}"
...
def generate_quarter(csv_path):
    ...
    source_base = target_path.stem
    target_base = financial_year_quarter_folder(source_base)
    target_period = quarter_label(source_base)

    quarter_output_dir = IPEA_OUTPUT_DIR / target_base
    quarter_output_dir.mkdir(parents=True, exist_ok=True)

    print(f"\nGenerating charts from source: {source_base}")
    print(f"Public reporting period: {target_base}")
